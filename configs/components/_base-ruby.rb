...
if platform.is_windows?
  ...
  #
  # Note that this step must happen _before_ the install step below.
  #
  # In Ruby 2.5.1, only rake must be wrapped this way:
  wrappers = %w{rake}
  if ruby_version_condensed.to_i < 251
    # But in Ruby < 2.5, all of these executables will need wrappers, too:
    wrappers.concat %w{erb gem irb rdoc ri}
  end

  pkg.install do
    wrappers.map do |name|
...
if platform.is_windows? && ruby_version_condensed.to_i > 250
  # Ruby >= 2.5 will generate cmd files instead of bat files for all but rake;
  # These cmd wrappers work fine in our environment if they're just renamed as
  # batch files.
  #
  # Note that this step must happen _after_ the install step above.
  pkg.install do
